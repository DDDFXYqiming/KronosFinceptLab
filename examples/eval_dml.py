...
# Load model on CPU first, then move to DML
...
print("Loading model on CPU (avoids pagefile issue)...", end=" ", flush=True)
t0 = time.perf_counter()
model = Kronos.from_pretrained(str(PROJ / "external" / "Kronos-small")).to(cpu_dev)
model.eval()
print(f"done ({time.perf_counter()-t0:.1f}s)")

print("Moving model to DML...", end=" ", flush=True)
t0 = time.perf_counter()
model.to(dml_dev)
tok.to(dml_dev)
print(f"done ({time.perf_counter()-t0:.1f}s)")
...
for pred_len, sc, T, p, note in grid:
    ...
    elapsed = time.perf_counter() - t0  # approximate
    print(f"  acc={acc*100:.1f}%  loss={loss:.4f}  ppl={ppl:.2f}  ({total} samples)", flush=True)
