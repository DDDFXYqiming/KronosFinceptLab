...
def _deepseek_chat_url() -> str:
    return _build_deepseek_chat_url(settings.llm.deepseek.base_url)
...
def classify_macro_request(
    ...
    *,
    ...
    """Classify a macro/cross-market question with DeepSeek first and local fallback second."""
...
{AGENT_SCOPE_DESCRIPTION}"""
        ...
        if not isinstance(parsed, dict):
            return None
        return _normalize_route_decision(
            parsed,
...
def _call_deepseek_macro_router(
    ...
    *,
    ...
    """Use DeepSeek only to route macro/cross-market questions, never to execute providers."""
...
{AGENT_SCOPE_DESCRIPTION}"""
        ...
        if not isinstance(parsed, dict):
            return None
        return _normalize_macro_route_decision(parsed, source="deepseek_macro_router")
    except Exception:
        return None
