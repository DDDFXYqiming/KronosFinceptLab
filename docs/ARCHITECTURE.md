...
graph TB
    ...
    MacroManager --> Kalshi
    MacroManager --> Deribit
    MacroManager --> OrderBook
    MacroManager --> Greeks
...
kronos_mcp/
├── kronos_mcp_server.py         # MCP 服务实现（33 个已注册工具）
└── __init__.py
```
...
项目包含 A 股与全球市场数据源、宏观信号提供方、常用技术指标和多类回测策略。具体数量会随实现变化，不作为架构契约；当前可调用能力以 [`API.md`](API.md)、[`CLI.md`](CLI.md) 和 [`../kronos_mcp/README.md`](../kronos_mcp/README.md) 为准。
