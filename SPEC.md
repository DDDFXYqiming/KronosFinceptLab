...
### 1. 测试要求
- **单元测试**: 每个新模块/函数必须有对应的单元测试
- **集成测试**: 模块间协作必须有集成测试覆盖
- **回归测试**: 新功能不能破坏已有功能，全量测试必须通过

### 2. 验收标准
- 全部测试通过（0 error）
- 代码无 emoji 输出（pre-commit hook 检查）
- CLI/API/Web 三种服务均可正常运行
- 核心功能手动验证通过

### 3. 验收流程
```
开发完成 → 单元测试 → 集成测试 → 全量回归 → 手动验收
                                                    ↓
                                              通过 → git commit + push
                                              不通过 → 修复 → 重新测试
```
...
### 4. 测试文件管理
- 测试脚本放在 `tests/` 目录
- 验收通过后，临时测试脚本（非 `tests/` 目录下的）必须删除
- `tests/` 下的正式测试保留，作为项目回归测试基线

### 5. Git 规范
- 验收通过后才能 commit + push
- commit message 必须明确版本号和功能摘要
- 推送前确认 `.env`、`external/`、`data/` 等敏感目录未被提交
