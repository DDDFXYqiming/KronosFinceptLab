...
V10 已推进到 v10.6.4：在 v10.6.3 轻量日志升级基础上，修复分析页模型调用稳定性。分析页 DeepSeek 汇总超时调整到 25s，避免 14~17s 正常响应被误判为失败；多标的 Agent Kronos 逐个复用预测页单标的预测路径，避免 batch 路径差异导致预测页正常但分析页失败。
