...
V10 已推进到 v10.7.1：分析页与宏观洞察页进入 OpenRouter Free 优先降级模式。配置 `OPENROUTER_API_KEY` 后，语义路由和最终报告汇总会先调用 `OPENROUTER_MODEL=openrouter/free`，失败后自动回退到 DeepSeek，再失败才使用本地结构化模板；线上 Web 入口对 OpenRouter/DeepSeek 调用增加墙钟超时预算，避免长请求被 Zeabur/Next 代理层断链。
