...
import {
  BarChart3, Bell, ChevronLeft, ChevronRight, Database,
  GitCompareArrows, Globe2, History, LayoutDashboard,
  LineChart, Settings, Star, type LucideIcon,
} from "lucide-react";
...
export function Sidebar() {
  ...
  const navElements = useMemo(() =>
    ...
    [segment, sidebarOpen, preferences.language],
  );
    ...
    >
      <div className={`flex h-16 items-center border-b border-border px-3 ${sidebarOpen ? "justify-between" : "justify-center"}`}>
        <Link href="/" className={`flex min-w-0 items-center ${sidebarOpen ? "gap-3" : "justify-center"}`} aria-label="KronosFinceptLab">
          <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-accent/10 bg-accent/5 font-display text-xl font-bold gradient-text">K</span>
          {sidebarOpen && <span className="truncate font-display text-lg gradient-text">KronosFinceptLab</span>}
        </Link>
        <button type="button" onClick={toggleSidebar}
          ...
          aria-label={t(preferences.language, "common.toggleSidebar")} aria-expanded={sidebarOpen}
